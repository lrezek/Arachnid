...
class Arachnid
{
    ...
    {
        //Keep track of the relations you have already deleted, so you don't try to delete them twice
        ...
        foreach($this->nodeEntitiesToRemove as $node)
        {
            ...
            if($id)
            {
                ...
                {
                    //Don't delete it if it's already been deleted in this batch!
                    if(!in_array($relationship->getId(), $deletedRelations))
                    {
                        $deletedRelations[] = $relationship->getId();
                        $relationship->delete();
                    }
                }
        ...
        foreach($this->relationEntitiesToRemove as $relation)
        {
            ...
            //If the relation has an id, and hasn't been deleted already in this batch, delete it!
            if($id && !in_array($id, $deletedRelations))
            {
                //Grab everyman relationship
    ...
    /**
     * Alternate notation for registerEvent().
     *
     * @param $eventName
     * @param $callback
     * @api
     */
    public function register_event($eventName, $callback)
    {
     ...
     * @api
     */
    public function createCypherQuery()
    {
        return new Query\Cypher($this);
    }

    /**
     * Alternate notation for createCypherQuery().
     *
     * @return Query\Cypher
     * @api
     */
    public function create_cypher_query()
    {
     ...
     */
    public function cypherQuery($string, $parameters)
    {
        try {
